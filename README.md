...
├── prompts/                   Generated verticals (8 industries)
│   ├── winners/               18-month production winner library (transfer-ranked)
│   └── industries/            Converting pack (opt-in + solar cold)
├── skills/
...
The eight generated verticals above are starting points. The prompts that actually transferred on the live platform (18 months of SuperAdmin history, generic labels only) live in a separate winner library: [prompts/winners/](prompts/winners/). Use those when you want the short, transfer-when-hot pattern instead of a long discovery script.

## Winner library (18-month production)

Six 12-section prompts reconstructed from the highest-transfer agents on the platform (Feb 2025 to Aug 2026). Ranked by live transfer and booked `appointment_date`, not by `call_successful`. Inbound wins on rate. Outbound wins on raw transfers. Transfer-when-hot is a conversion. Client names stripped. See [prompts/winners/](prompts/winners/) and [WHY-THEY-WON](prompts/winners/WHY-THEY-WON.md).

### Converting pack (opt-in + solar cold)

Hand-built converting templates live in [prompts/industries/](prompts/industries/). Solar is the flagship (cold, opt-in, inbound) with a 2026 post-ITC FAQ. The rest of the pack is inbound and outbound-optin for the industries this library actually serves. Same 12 sections. How-are-you-then-reason. One-sentence AI disclosure. Book or transfer.

These files do not replace the generated verticals above. They also do not rewrite the medical, medspa, or dental winners. Start with [prompts/industries/WHY-THESE-CONVERT.md](prompts/industries/WHY-THESE-CONVERT.md). Folder index: [prompts/industries/README.md](prompts/industries/README.md).

The CLI still emits the generated verticals only. It does not rewrite this pack.
